...
import {AggregatorV2V3Interface} from "src/pool/interfaces/AggregatorV2V3Interface.sol";

import {PRICEv1} from "src/pool/modules/PRICE/PRICE.v1.sol";
import "src/pool/Kernel.sol";
...
contract OlympusPrice is PRICEv1 {
    ...
    constructor(
        ...
        /// @dev Moving Average Duration should be divisible by Observation freq to get a whole number of observations
        if (
    ...
    function getCurrentPrice() public view override returns (uint256) {
        ...
        uint256 reserveEthPrice;
        {
            ...
            (
                ...
                updatedAt,
                answeredInRound
            ) = reserveEthPriceFeed.latestRoundData();
            if (
                reserveEthPriceInt <= 0 ||
                updatedAt <
                block.timestamp - uint256(reserveEthUpdateThreshold) ||
                answeredInRound != roundId
    ...
    function getTargetPrice() external view override returns (uint256) {
        ...
        return
            movingAverage > minimumTargetPrice
                ? movingAverage
                : minimumTargetPrice;
    }
}
